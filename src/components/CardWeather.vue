...
const removeCard = (city, event) => {
  swal({
    title: `You delete the card about ${city?.location?.name}, are you sure?`,
    text: `The card contains information about the city of ${city?.location?.name}`,
    icon: "warning",
    buttons: true,
    dangerMode: true
  })
    .then((willDelete) => {
      if (willDelete) {
        let cities = JSON.parse(localStorage.getItem('cityNames'))
        ...
        swal(`Poof! Your ${city?.location?.name} city card has been deleted!`, {
          icon: "success"
        })
      }
    })
  ...
  <div class="cards" v-if="weatherList.length">
    ...
    >
      ...
      <div v-show="activeCard !== city.id" class="card-main">
        <button @click="removeCard(city, $event)" class="card-btn card-btn--remove"><span class="material-icons">delete</span>
        </button>
        <button
          @click="toggleCard(city, $event)"
